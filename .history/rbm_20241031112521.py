...
def predictRatingMax(ratingDistribution):
    ...
    return np.argmax(ratingDistribution, axis=-1, keepdims=True) + 1 # + 1 to convert from 0-4 to 1-5
...
def predictRatingExp(ratingDistribution):
    ...
    weighted_average_rating = np.sum(np.multiply(ratingDistribution, ratings_of_same_shape), axis=-1, keepdims=True)
