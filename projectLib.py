...
def getTrainingData():
    return np.genfromtxt("training.csv", delimiter=",", dtype=np.int32)

def getValidationData():
    return np.genfromtxt("validation.csv", delimiter=",", dtype=np.int32)
...
def getAllUsersRatings(users, training):
    return [getRatingsForUser(user, training) for user in users]
